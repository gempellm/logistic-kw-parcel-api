...
```sh
[I] ➜ grpc_cli call localhost:8082 DescribeParcelV1 "id: 1"
connecting to localhost:8082
Rpc failed with status code 5, error message: parcel not found
```
...
```sh
[I] ➜ curl -s -X 'POST' \
  'http://localhost:8080/v1/parcels' \
...
Apache Kafka is an open-source distributed event streaming platform used by thousands of clogisticanies for high-performance data pipelines, streaming analytics, data integration, and mission-critical applications.
...
Monitor and troubleshoot transactions in clogisticlex distributed systems.
...
```sh
$ pgcli "postgresql://docker:docker@localhost:5432/logistic_parcel_api"
```
