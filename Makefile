...
.PHONY: generate
generate: .generate-go .generate-python .generate-finalize-go .generate-finalize-python

.PHONY: generate
generate-go: .generate-go .generate-finalize-go

#@ command -v buf 2>&1 > /dev/null || 
#@ command rm -f "$(BUF_EXE)" && echo "Install buf" &&
.generate-install-buf:
	@ command echo "Install buf" && \
    		curl -sSL https://github.com/bufbuild/buf/releases/download/$(BUF_VERSION)/buf-$(OS_NAME)-$(OS_ARCH)$(shell go env GOEXE) --create-dirs -o "$(BUF_EXE)" && \
    		chmod +x "$(BUF_EXE)"
...
.deps-python:
	pip3 install grpcio-tools grpclib[protobuf] protobuf
